...
static std::string getNick() {
	std::string nick;
	std::cout << "Enter you Nick_Name to prompting in chat: ";
	std::getline(std::cin, nick);
	if (nick.empty()) sys_er("getPrompt: cannot save nick_name");
	std::transform(nick.begin(), nick.end(), nick.begin(), ::tolower);
	return nick;
}

static void connect_socket(tcp::socket &socket, tcp::endpoint &ep) {
	EXCEPT_HANDLE(socket.connect(ep, er_code););
}

static void readFromSocket(tcp::socket *socket) {
	constexpr size_t bufferSize = 1024;
	std::size_t      readBytes  = 0;

	while (RUN_CHAT) {
		EXCEPT_HANDLE(if (socket->available(er_code)) {
			char readBuffer[bufferSize] = { 0 };
			readBytes                   = socket->read_some(
                boost::asio::buffer(readBuffer, bufferSize), er_code);
...
static void addTime(std::string &input) {
	char       buf[15]     = { 0 };
...
static void writeToSocket(tcp::socket *socket) {
	while (RUN_CHAT) {
		...
		if (!inputBuffer.empty())
			EXCEPT_HANDLE(
			    socket->write_some(boost::asio::buffer(inputBuffer), er_code););
...
int main() {
	...
	tcp::endpoint endpoint(boost::asio::ip::address::from_string(SERVER_NAME),
	                       PORT_NUM);
